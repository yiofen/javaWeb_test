	...
	<body>
					...
					<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a data-toggle="collapse" data-parent="#accordion" href="#Radio">单选题（共 <c:out value="${fn:length(exams)}"></c:out> 题，每题2分）</a>
							</h4>
						</div>
								...
								<div class="panel-body">
									<ol>
										<c:forEach items="${exams}" var="item" varStatus="status">
											<div class="subject" data-i="${status.index}" data-answer="false" data-eid="${item.eid}" data-key="${item.ekey}" data-state="0" data-skey>
												<li> ${item.etitle}</li>
												<ol>
													<li><label><input type="radio" value="A" name="${item.eid}">${item.ea}</label></li>
													<li><label><input type="radio" value="B" name="${item.eid}">${item.eb}</label></li>
													<li><label><input type="radio" value="C" name="${item.eid}">${item.ec}</label></li>
													<li><label><input type="radio" value="D" name="${item.eid}">${item.ed}</label></li>
												</ol>
											</div>
										</c:forEach>
									</ol>
		...
		<script>
			...
			function getScore(){
				//var spid = n;
				$.post({
			        url: basePath + 'user/studPaper?cmd=score&uid=${sessionScope.user.sid}&spid='+ now.getTime(),
			        contentType: false,
			        processData: false,
			        success: function(res) {
			            ...
			            layer.open({
							...
							icon:1,
								...
								location.href = basePath+'user/studPaper?cmd=studpaper';
							}
						})
			        },
			        error: function(res) {
			...
			function postAnswer(){
				...
				var pname = '${pname}';
				var uid = '${sessionScope.user.sid}';
				$('.subject').each(function(index){
					var i = index;
					var self = $(this);
					var data = {uid:uid,eid:self.data('eid'),studkey:self.data('skey'),studstate:self.data('state'),pname:pname,spid:now.getTime()}
					var datas = $.param(data)
					console.log(datas);
					$.post({
						url: basePath+'user/studPaper?cmd=answer&'+ datas,
						contentType: false,
						processData: false,
						success:function(){
							// 在最后一次提交完成后跳转到错题列表
